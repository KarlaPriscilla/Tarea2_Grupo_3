...
// SOLUCI?N 1: Delay manual con bucles
void custom_delay_ms(unsigned int ms) {
    unsigned int i, j;
    for (i = 0; i < ms; i++) {
        for (j = 0; j < 1000; j++) {
            asm nop;  // Ajusta este valor seg?n tu frecuencia de oscilador
        }
    }
}
...
void custom_delay_us(unsigned int us) {
    unsigned int i;
    for (i = 0; i < us; i++) {
        asm nop;  // Ajusta seg?n tu oscilador
    }
}
...
// SOLUCI?N 2: Funci?n con casos espec?ficos para delays comunes
...
// Funci?n para tocar una nota musical con delay personalizado
...
// SOLUCI?N 3: Versiones espec?ficas para cada sonido
void beep_short() {
    unsigned short i;
    for (i = 0; i < 2; i++) {
        BUZZER = 1;
...
// Melod?a de inicio estilo Pac-Man
void startup_melody() {
...
// Melod?a alternativa estilo retro gaming
void retro_startup() {
...
// Sonido de comida usando funci?n espec?fica
void waka_waka_sound() {
    beep_short();  // O usa: beepPattern_fixed(2, 1);
}

// Sonido de game over usando funci?n espec?fica
...
void main() {
    ...
    TRISD.TRISD1 = 1;       // RD1 como entrada (bot?n comida)
    TRISD.TRISD2 = 1;       // RD2 como entrada (bot?n game over)
    ...
    // Selecciona UNA de estas melod?as:
    startup_melody();       // Melod?a principal
